...
def aa_probs_from_masked_token_distributions(
    ...
    if aa_pos not in site_to_pos_aas or len(site_to_pos_aas[aa_pos]) == 0:
        raise ValueError(f"site_to_pos_aas[{aa_pos}] missing/empty")

    wt_aa = str(site_to_pos_aas[aa_pos][0]).upper()
    allowed_set = set(str(x).upper() for x in site_to_pos_aas[aa_pos])

    dna = dna_seq.upper()
    s0 = int(mask_span[0])
    if s0 < 0 or s0 + 3 > len(dna):
        raise ValueError("mask_span start out of range for original dna_seq")
    ...
    if len(probs_list) == 1:
        ...
        best = {aa: 0.0 for aa in allowed_set}
        probs = probs_list[0]
        ...
        for tok, p in zip(tokens, probs):
            ...
            if s0 + 3 > len(new_dna):
                continue
            aa = translate_codon(new_dna[s0:s0+3])
            if aa in best:
                pv = float(p)
                if pv > best[aa]:
                    best[aa] = pv
    ...
    # ---- 2 masks
    i1, i2 = masked_token_positions
    (s1, e1) = offsets[i1]
    (s2, e2) = offsets[i2]
    p1 = np.asarray(probs_list[0], dtype=float)
    p2 = np.asarray(probs_list[1], dtype=float)
    ...
    if e1 > s2:
        raise ValueError("masked token spans overlap; expected 2 distinct tokens")

    old1 = e1 - s1
    old2 = e2 - s2

    valid_idx = [k for k, t in enumerate(tokens) if t is not None]
    toks = [tokens[k].upper() for k in valid_idx]
    p1v = p1[valid_idx]
    p2v = p2[valid_idx]
    Ls = np.array([len(t) for t in toks], dtype=int)

    distinct_L2 = sorted(set(int(L) for L in Ls))
    pos_list = [s0, s0+1, s0+2]

    # max p2 per length (для req==0)
    max_p2_by_L2 = {}
    for Lx, pv in zip(Ls, p2v):
        Lx = int(Lx); pv = float(pv)
        if pv > max_p2_by_L2.get(Lx, 0.0):
            max_p2_by_L2[Lx] = pv

    # ограничим индексы максимумом длины токена
    max_tok_len = int(Ls.max())

    deltas1 = sorted(set(int(L) - old1 for L in Ls))
    needed_idx_sets = {}
    for d1 in deltas1:
        start2 = s2 + d1
        idxs = []
        for pos in pos_list:
            idx = pos - start2
            if 0 <= idx < max_tok_len:
                idxs.append(idx)
        needed_idx_sets[d1] = tuple(sorted(set(idxs)))

    all_needed_idx_tuples = set(needed_idx_sets.values())

    best2_1, best2_2, best2_3 = {}, {}, {}
    tok_chars = [list(t) for t in toks]

    for chars, L2, prob in zip(tok_chars, Ls, p2v):
        pv = float(prob)
        L2 = int(L2)
        for idx_tuple in all_needed_idx_tuples:
            if len(idx_tuple) == 0:
                continue
            if max(idx_tuple) >= L2:
                continue
            ...
            if len(idx_tuple) == 1:
                ...
                b0 = chars[i0]
                key = (L2, int(i0), b0)
                if pv > best2_1.get(key, 0.0):
                    best2_1[key] = pv
            elif len(idx_tuple) == 2:
                i0, i1 = idx_tuple
                b0, b1 = chars[i0], chars[i1]
                key = (L2, int(i0), b0, int(i1), b1)
                if pv > best2_2.get(key, 0.0):
                    best2_2[key] = pv
            else:
                i0, i1, i2 = idx_tuple
                b0, b1, b2 = chars[i0], chars[i1], chars[i2]
                key = (L2, int(i0), b0, int(i1), b1, int(i2), b2)
                if pv > best2_3.get(key, 0.0):
                    best2_3[key] = pv

    best = {aa: 0.0 for aa in allowed_set}

    for tok1, L1, prob1 in zip(toks, Ls, p1v):
        pa = float(prob1)
        if pa == 0.0:
            continue
        ...
        def char_after_tok1(pos):
            if pos < s1:
                return dna[pos]
            if pos < s1 + L1:
                return tok1[pos - s1]
            old_pos = pos - d1
            return dna[old_pos] if 0 <= old_pos < len(dna) else None

        for L2 in distinct_L2:
            d2 = int(L2) - old2
            ...
            for pos in pos_list:
                if pos < start2:
                    ch = char_after_tok1(pos)
                    if ch is None:
                        ok_len = False; break
                    src.append(("fixed", ch))
                elif pos < start2 + int(L2):
                    src.append(("tok2", pos - start2))
                else:
                    ch = char_after_tok1(pos - d2)
                    if ch is None:
                        ok_len = False; break
                    src.append(("fixed", ch))
            if not ok_len:
            ...
            fixed0 = src[0][1] if src[0][0] == "fixed" else None
            fixed1 = src[1][1] if src[1][0] == "fixed" else None
            fixed2 = src[2][1] if src[2][0] == "fixed" else None

            for b0 in "ACGT":
                if fixed0 is not None and b0 != fixed0: continue
                for b1 in "ACGT":
                    if fixed1 is not None and b1 != fixed1: continue
                    for b2 in "ACGT":
                        if fixed2 is not None and b2 != fixed2: continue

                        aa = translate_codon(b0 + b1 + b2)
                        if aa not in best:
                            continue

                        req = []
                        if src[0][0] == "tok2": req.append((src[0][1], b0))
                        if src[1][0] == "tok2": req.append((src[1][1], b1))
                        if src[2][0] == "tok2": req.append((src[2][1], b2))

                        if len(req) == 0:
                            p2_best = max_p2_by_L2.get(int(L2), 0.0)
                        elif len(req) == 1:
                            (i0, c0) = req[0]
                            p2_best = best2_1.get((int(L2), int(i0), c0), 0.0)
                        elif len(req) == 2:
                            (i0, c0), (i1, c1) = req
                            p2_best = best2_2.get((int(L2), int(i0), c0, int(i1), c1), 0.0)
                        else:
                            (i0, c0), (i1, c1), (i2, c2) = req
                            p2_best = best2_3.get((int(L2), int(i0), c0, int(i1), c1, int(i2), c2), 0.0)
...
def main(args):
    ...
    corr_mode = "median"
    save_path=f"{args.out_dir}/{args.name}_{corr_mode}_fnscore_vs_fscore_filtr.png"
    ...
    corr_mode = "median"
    save_path=f"{args.out_dir}/{args.name}_{corr_mode}_fnscore_vs_fscore_logscale_filtr.png"
