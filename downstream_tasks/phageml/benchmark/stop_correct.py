...
def build_modergena_model(checkpoint_filepath, modernbert_distr_path, name, random_init):
    ...
    cfg_path = os.path.join(cpt_dir, "cfg.yaml")#f"{name}.txt")
...
def random_order_sampling(model,
    ...
    for seq in sequences:
        ...
        for pos in order:
            ...
            #print(old_id)
            #new_id = 0 ### test ckpt
            #while new_id < 128 or new_id > 5588: ### test ckpt
            #while new_id < 6 or new_id > 32000: ### test ckpt 
            with torch.no_grad():
            ...
            if model_name == 'kmer':
                ...
                for bad_id in [tokenizer.cls_token_id, tokenizer.sep_token_id,
                            tokenizer.pad_token_id, tokenizer.mask_token_id, 5]:
                    probs[bad_id] = 0.0
            ...
            print(new_id, tokenizer.convert_ids_to_tokens(new_id))
...
def main(args):
    ...
    sequences = [fasta.fetch(reference=chrom, start=start, end=end).upper()] #* 20
    ...
    #             'GTCAGGGACTATGTG',]
    model_name = 'kmer' if 'kmer' in str(args.name) else 'bpe'
    ...
    #print(np.median(arr))
    '''
    unique_tokens = [len(set(i)) for i in metrics['token_distr']]
    ...
    print('same')
    print(np.mean(metrics['same_token']))
    print(np.median(metrics['same_token']))

    print('unique')
    print(np.mean(unique_tokens))
    print(np.median(unique_tokens))
    raise
    '''
