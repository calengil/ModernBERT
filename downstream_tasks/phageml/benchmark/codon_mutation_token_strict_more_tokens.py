...
def build_window_around_gene(
    ...
    gene_start_pos = 0 + len(left_part)
    ...
    return input_ids, attention_mask, inner_positions


def tokenize_with_offsets(tokenizer, fasta, cds):
    """
    ...
    ids, attn, inner_positions = build_window_around_gene(
                                        fasta,                  # pysam.FastaFile
                                        tokenizer,
                                        chrom=cds[0],
                                        gene_start=cds[1],         # 0-based
                                        gene_end=cds[2],           # 0-based half-open
    ...
    gene_start = offsets[min(inner_positions)][0]

    #print(f"first_tok = {tokenizer.convert_ids_to_tokens(ids[inner_positions[0]])}")
    #print(f"last_tok = {tokenizer.convert_ids_to_tokens(ids[inner_positions[-1]])}")

    #print(dna_seq[gene_start: gene_start+3])
    #raise
    return ids, attn, toks, offsets, dna_seq, gene_start
...
def aa_probs_from_masked_token_distributions(
    ...
    # ---- 1 mask
    print(f"len(probs_list) = {len(probs_list)}")
    if len(probs_list) == 1:
...
def collect_prob(model,
    ...
    ids_no_special, attn_no_special, toks_no_special, offsets, dna_seq, cds_start = tokenize_with_offsets(tokenizer, fasta, cds)
    dna_seq = dna_seq.upper()
    ...
    for aa_pos in tqdm(site_groups):
        ...
        print(group_substitution_fn_scores)
        print(aa_pos)
        raise
        #print('fn')
