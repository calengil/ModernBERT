...
def load_and_prepare_mutation_csv(
    ...
    # Normalize each library by its own WT
    for col in library_cols:
        wt_value = df.loc[wt_mask, col].values[0]
        df[col] = df[col] / wt_value

    # Remove WT after normalization
    df = df[~wt_mask].copy()
...
def select_split_with_library_priority(
    ...
    """
    Builds one split with one library preferred over the other.

    Logic:
    - take rows of the requested split by single_mut
    - first take all Mutation rows where preferred_library_col > 0
    - then add rows from fallback_library_col only for Mutation values
      that were not already taken from the preferred library

    Returns a dataframe where:
    - each row = one sample
    - column selected_score contains the chosen score
    - column source_library contains the selected library name
    """
...
def parse_mutation_string(mutation: str) -> list[tuple[str, int, str]]:
    """
    Parses mutation string:
      A12G -> [('A', 12, 'G')]
      A12G_L45P_R100K -> [('A', 12, 'G'), ('L', 45, 'P'), ('R', 100, 'K')]
    """
    if mutation is None or pd.isna(mutation):
        return []
    ...
    parsed_parts: list[tuple[str, int, str]] = []
    for part in mut.split("_"):
        m = _MUT_RE.match(part.strip())
        if m is None:
            raise ValueError(f"Bad mutation format: {part} (full mutation={mut})")

        aa_from = m.group(1).upper()
        pos = int(m.group(2))
        aa_to = m.group(3).upper()
        parsed_parts.append((aa_from, pos, aa_to))

    return parsed_parts


def mutation_hits_region(
    mutation: str,
    region: tuple[int, int],
) -> bool:
    """
    Returns True if at least one mutation position falls
    into [region_start, region_end] inclusive.
    """
    region_start, region_end = region
    if region_start > region_end:
        raise ValueError(f"Bad region: start={region_start} > end={region_end}")

    parsed_parts = parse_mutation_string(mutation)
    return any(region_start <= pos <= region_end for _, pos, _ in parsed_parts)


def filter_mutations_by_region(
    df: pd.DataFrame,
    region: tuple[int, int],
    mutation_col: str = "Mutation",
) -> pd.DataFrame:
    """
    Keeps only rows where at least one Mutation position
    falls into the inclusive region.
    """
    if mutation_col not in df.columns:
        raise ValueError(f"Column '{mutation_col}' not found")

    mask = df[mutation_col].apply(lambda x: mutation_hits_region(x, region))
...
def make_most_frequent_codon_mutants(
    ...
    """
    For each row of df creates one sample:
      - if Mutation = A12G, one codon is changed
      - if Mutation = A12G_L45P_R100K, several codons are changed

    For each aa_to, the most frequent codon from aa2codons_freq is chosen.

    Returns:
      mut_seqs: mutated DNA sequences
      fn_scores: corresponding scores
      mut_metadata: mutation strings for metadata
    """
    if mutation_col not in df.columns:
        raise ValueError(f"Column '{mutation_col}' not found")
    ...
    for _, row in df.iterrows():
        ...
        parsed_parts = parse_mutation_string(mut)
        ...
        for _, pos, aa_to in parsed_parts:
        ...
        fn_scores.append(v)
        mut_metadata.append(mut)

    return mut_seqs, fn_scores, mut_metadata
...
# Inclusive region for TEST
if mode == 'first':
    test_region = (124, 190) #first
elif mode == 'second':
    test_region = (348, 409) #second
elif mode == 'third':
    test_region = (483, 549) #third


# Common filters for both libraries
df_filter = {}
...
# 1) Simple mutations: same logic as before for train
simple_train_df = select_split_with_library_priority(
    mutation_df,
...
# 2) Complex mutations: same logic as before
complex_df = select_split_with_library_priority(
    mutation_df,
...
# Test:
#   - only complex mutations
#   - at least one position falls in test_region
test_df = filter_mutations_by_region(
    complex_df,
    region=test_region,
    mutation_col="Mutation",
)
...
# Train:
#   - all simple mutations
#   - all complex mutations EXCEPT those included in test
test_mutations = set(test_df["Mutation"].astype(str))
complex_train_df = complex_df[~complex_df["Mutation"].astype(str).isin(test_mutations)].copy().reset_index(drop=True)

train_df = pd.concat([simple_train_df, complex_train_df], axis=0, ignore_index=True).reset_index(drop=True)

print(f"simple train samples = {len(simple_train_df)}")
print(simple_train_df["source_library"].value_counts(dropna=False).to_dict())

print(f"complex train samples = {len(complex_train_df)}")
print(complex_train_df["source_library"].value_counts(dropna=False).to_dict())

print(f"train samples total = {len(train_df)}")
print(train_df["source_library"].value_counts(dropna=False).to_dict())

print(f"test region = {test_region[0]}..{test_region[1]} (inclusive)")
...
train_mut_seqs, train_fn_scores, train_metadata = make_most_frequent_codon_mutants(
...
test_mut_seqs, test_fn_scores, test_metadata = make_most_frequent_codon_mutants(
    aa2codons_freq=aa2codons_freq,
    dna_seq=sequence,
    cds_start_0based=cds_start,
    df=test_df,
...
plot_distribution(
      train_fn_scores,
      f'/home/jovyan/shares/SR003.nfs2/caduseus_artem/phageml/ModernBERT/downstream_tasks/phageml/datasets/partial_multitest/plots/train_v2_positive_{mode}_log.png'
)
plot_distribution(
      test_fn_scores,
      f'/home/jovyan/shares/SR003.nfs2/caduseus_artem/phageml/ModernBERT/downstream_tasks/phageml/datasets/partial_multitest/plots/test_v2_positive_{mode}_log.png'
)
#raise
output_path = '/home/jovyan/shares/SR003.nfs2/caduseus_artem/phageml/ModernBERT/downstream_tasks/phageml/datasets/partial_multitest'

train_output = f'{output_path}/train_v2_positive_{mode}.hdf5'
with h5py.File(train_output, "a") as file:
    for i, fn in tqdm(enumerate(train_fn_scores), total=len(train_fn_scores), desc='Processing train'):
        ...
        group.attrs['metadata'] = train_metadata[i]
        group.create_dataset("fn_transform", data=np.array([float(fn)], dtype=np.float64))

test_output = f'{output_path}/test_v2_positive_{mode}.hdf5'
with h5py.File(test_output, "a") as file:
    for i, fn in tqdm(enumerate(test_fn_scores), total=len(test_fn_scores), desc='Processing test'):
        ...
        group.attrs['metadata'] = test_metadata[i]
        group.create_dataset("fn_transform", data=np.array([float(fn)], dtype=np.float64))