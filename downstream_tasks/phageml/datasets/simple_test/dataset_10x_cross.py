...
fold_list = []
for n in range(10):
    fold = [i for i in range(n+1, len(sequence)//3 + 1, 10)]
    fold_list.append(fold)

fold_list_len = [len(i) for i in fold_list]
print(fold_list_len)

import matplotlib.pyplot as plt
def plot_distribution(data, save_path):
    # data — список из 10 массивов
    n_plots = len(data)
    fig, axes = plt.subplots(2, 5, figsize=(20, 8))  # 2 ряда, 5 колонок
    axes = axes.flatten()  # Преобразовать в 1D для удобства
    
    for i in range(n_plots):
        ax = axes[i]
        ax.hist(data[i], bins=20, range=(0, 1), edgecolor='black', alpha=0.7, density=True)
        ax.set_xlabel('F1-score transformed')
        ax.set_ylabel('Density')
        ax.set_title(f'Fold {i+1}')
        ax.grid(True)
    
    # Убрать пустые subplots если <10
...
for fold in fold_list:
    ...
    dataset_list_fn_scores.append(test_fn_scores)
    dataset_list_seqs.append(test_fn_scores)

#plot_distribution(dataset_list_fn_scores, '/home/jovyan/shares/SR003.nfs2/caduseus_artem/phage/ModernBERT/finetuning/datasets/test_dataset/10x_cross.png')

dataset_fold_list_len = [len(i) for i in dataset_list_fn_scores]
#print(dataset_fold_list_len)



output_path = '/home/jovyan/shares/SR003.nfs2/caduseus_artem/phage/ModernBERT/finetuning/datasets/test_dataset'
train_output = f'{output_path}/10x_cross.hdf5'

with h5py.File(train_output, "a") as file:

    for f in range(len(dataset_list_fn_scores)):
        fold = file.create_group(f"fold_{f}")
        
        for i, fn in tqdm(enumerate(dataset_list_fn_scores[f]), total=len(dataset_list_fn_scores[f]), desc=f'Processing fold {f}'):
            group = fold.create_group(f"sample_{i}") 
            group.attrs['seq'] = dataset_list_seqs[f][i]
