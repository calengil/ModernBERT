...
def load_and_filter_mutation_csv(
    ...
    if isinstance(first_library_col, int):
        ...
        first_lib_name = df.columns[first_lib_idx]
    else:
        first_lib_name = first_library_col
        first_lib_idx = df.columns.get_loc(first_lib_name)

    keep_cols = df.columns[:first_lib_idx + 1].tolist()

    wt_mask = df['Mutation'] == 'WT'

    wt_value = df.loc[wt_mask, first_lib_name].values[0]
    ...
    df[first_lib_name] = df[first_lib_name] / wt_value

    df = df[~wt_mask]
    mask &= (df[first_lib_name] > 0) ################### nonzero
    df = df[mask]
...
#raise
#plot_distribution(train_fn_scores, '/home/jovyan/shares/SR003.nfs2/caduseus_artem/phageml/ModernBERT/downstream_tasks/phageml/datasets/simple_test/plots/train_v2_positive_log.png')
#plot_distribution(test_fn_scores, '/home/jovyan/shares/SR003.nfs2/caduseus_artem/phageml/ModernBERT/downstream_tasks/phageml/datasets/simple_test/plots/test_v2_positive_log.png')
#raise
